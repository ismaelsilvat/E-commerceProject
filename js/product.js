...
function img2() {
    document.getElementById("imagem-principal").src = produto.imagens[1]
}

function img3() {
    document.getElementById("imagem-principal").src = produto.imagens[2];
}
...
let produtoEmJsonCodificado = params.get('product')
console.log(produtoEmJsonCodificado);

const produtoEmJson = decodeURIComponent(window.atob(produtoEmJsonCodificado));
console.log("PROFUTO EM JSON decodificado", produtoEmJson);

const produto = JSON.parse(produtoEmJson)
const descricao = decodeURIComponent(produto.descricao);
