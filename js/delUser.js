...
var pessoasCadastro = JSON.parse(localStorage.getItem('Users'))
...
function localizarPessoas() {
    ...
    for (let i = 0; i < pessoasCadastro.length; i++) {
        ...
        if (pessoasCadastro[i].nome == inputs[0].value || pessoasCadastro[i].email == inputs[0].value) {
    ...
    for (let i = 0; i < pessoasCadastro.length; i++) {
        ...
        if (pessoasCadastro[i].nome == inputs[0].value || pessoasCadastro[i].email == inputs[0].value) {
            ...
            inputs[0].value = ''
            inputs[1].value = pessoasCadastro[i].nome
...
function removerPessoas() {
    ...
    for (let i = 0; i < pessoasCadastro.length; i++) {
        if (pessoasCadastro[i].nome == inputs[1].value && pessoasCadastro[i].email == inputs[2].value) {

            indice = i;
            pessoasCadastro.splice(indice, 1);
            localStorage.setItem('Users', JSON.stringify(pessoasCadastro))
